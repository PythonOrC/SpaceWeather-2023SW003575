...
for name in names:
    ...
    data = np.abs(data)
    sns.heatmap(data, mask = msk, annot=False,fmt=fmt, color='white',xticklabels=list(data.columns), yticklabels=list(data.columns),cmap=sns.color_palette(cm, as_cmap=True), vmin = 0,vmax = lim_max, cbar_kws={'label': label})
