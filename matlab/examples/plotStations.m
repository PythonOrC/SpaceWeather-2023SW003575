% TODO: sort stations by latitude then plot
% TODO: plot by a line of various latitude, ~15 longitude, prenoon
...
STATIONS = ["B04"  "B11"  "B12"  "B15"  "B19"  "B20"  "B21"  "B22"  "CHD"  "CSY"  "DRV"  "HRN"  "KEV"  "KTN"  "LYR"  "MAW"  "MCQ"  "MGD"  "NAL"  "NOK"  "PAF"  "RVK"  "SBA"  "SOR"  "TIK"  "TRO"]; 
...
OBS = sortrows(OBS, lat);
filtered = {};
for i = 1:length(Stations)
    for j = 1:length(STATIONS)
        if strcmp(string(Stations(i)), string(STATIONS(j)))
            filtered = [filtered; OBS.data{i}(:)];
        end
    end
end
figure;
...
for i = 1:length(STATIONS)
    ...
    plot(1:length(data_f),data_f(:)+SPACING*i);
end
